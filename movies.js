firebase.auth().onAuthStateChanged(async function(user) {
  ...
  let apiKey = '5103a7cb41fa9394b2db2041fb884ef5'
  ...
  Hi ${userId}! Welcome to your watchlist.<br>
  `

  // Sign-out button (note there is a +=)
  document.querySelector('.sign-in-or-sign-out').innerHTML += `
  ...
  for (let i=0; i<movies.length; i++) {
    let movie = movies[i]
    let docRef = await db.collection(`watched-${uId}`).doc(`${movie.id}`).get()
    ...
    document.querySelector(`.movie-${movie.id}`).addEventListener('click', async function(event) {
      ...
      await db.collection(`watched-${uId}`).doc(`${movie.id}`).set({})
    }) 
  } // end of for loop

  } // lines up with if (user) {
  else {
...
}) // lines up with firebase.auth().onAuthStateChanged(async function(user) {
